...
// Classify a hex colour as green or red by RGB dominance. This is the only
// signal the centre emoji uses — Admin controls which answer option is green
// and which is red, so the emoji follows that configuration and never inspects
// the Yes/No answer text. A colour that is clearly neither green nor red
// returns null.
...
// The centre emoji counts how many of the three selected answers are coloured
// green vs red, using the persisted Admin-configured answer colour
// (optionColor). It never reads the answer text, so a "Yes" that Admin coloured
// red counts as red.
function getCenterEmoji(data) {
  const greenCount = data.filter((d) => classifyAnswerColour(d.optionColor) === 'green').length;
  const redCount = data.filter((d) => classifyAnswerColour(d.optionColor) === 'red').length;
...
export default function ResultScreen({ result, onKY, onAbout, onLogoClick, onRetake, onLogout }) {
  ...
  const buildDisplay = (slot, score) => {
    ...
    const optionColor = score?.optionColor ?? score?.selectedOptionColor ?? score?.stage?.color ?? null;
    const displayColor = optionColor || NEUTRAL_COLOR;
    return {
